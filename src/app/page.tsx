...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        const maintenance = json?.settings?.maintenanceMode;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden p-4 md:p-8 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] bg-[#0b0613]">
      ...
      {(accessChecking || (accessGateEnabled && !accessGranted)) && !maintenanceMode && (
      ...
      {maintenanceMode && (
        <div className="fixed inset-0 z-80 flex items-center justify-center px-4 py-8 bg-[#0b0613]">
          <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(900px 450px at 50% 0%, rgba(236,72,153,0.45), transparent 60%), radial-gradient(700px 420px at 85% 25%, rgba(168,85,247,0.40), transparent 55%)' }} aria-hidden="true" />
          <div className="relative w-full max-w-md rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl p-8 text-white text-center">
            <div className="mx-auto w-16 h-16 rounded-2xl bg-amber-500/15 text-amber-500 flex items-center justify-center mb-6">
              <Zap size={32} />
            </div>
            <h2 className="text-3xl font-bold mb-3 bg-linear-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Maintenance Mode</h2>
            <p className="text-white/70 mb-8 leading-relaxed">
              Kami sedang melakukan perbaikan sistem untuk memberikan layanan yang lebih baik. Silakan kembali lagi nanti.
            </p>
            <div className="text-xs text-white/40 border-t border-white/10 pt-6">
              © 2025 QINZ STORE • Layanan Email Sementara
            </div>
          </div>
        </div>
      )}
